export const GET_ALL_PRODUCTS = "GET_ALL_PRODUCTS";
export const GET_DELETED_PRODUCTS = "GET_DELETED_PRODUCTS";
export const TOGGLE_PRODUCT_TYPE = "TOGGLE_PRODUCT_TYPE";
export const TOGGLE_DELETED_PRODUCT_TYPE = "TOGGLE_DELETED_PRODUCT_TYPE";
export const RESTORE_PRODUCT = "RESTORE_PRODUCT";
export const SEARCH_PRODUCT_BY_NAME = "SEARCH_PRODUCT_BY_NAME";
export const SEARCH_PRODUCT_BY_ID = "SEARCH_PRODUCT_BY_ID";
export const CLEAR_PRODUCTS = "CLEAR_PRODUCTS";
export const GET_CURRENT_USER = "GET_CURRENT_USER";
export const USER_LOGIN = "USER_LOGIN";
export const USER_LOGOUT = "USER_LOGOUT";
export const ORDER_NAME = "ORDER_NAME";
export const ORDER_PRECIO = "ORDER_PRECIO";
export const POST_IMAGE = "POST_IMAGE";
export const POST_PRODUCT = "POST_PRODUCT";
export const UPDATE_PRODUCT = "UPDATE_PRODUCT";
export const DELETE_PRODUCT = "DELETE_PRODUCT";
export const GET_ALL_USERS = "GET_ALL_USERS";
export const POST_ADMIN = "POST_ADMIN";
export const GET_CART_PRODUCTs = "GET_CART_PRODUCTS";
export const ADD_CART_PRODUCTS = "ADD_CART_PRODUCTS";
export const GET_CART_PRODUCTS = "GET_CART_PRODUCTS";
export const ADD_FAVORITO_PRODUCT = "ADD_FAVORITO_PRODUCT";
export const GET_USER = "GET_USER";
export const REGISTER_USER = "REGISTER_USER";
export const CLEAR_CART_PRODUCTS = "CLEAR_CART_PRODUCTS";
export const DELETE_CART_PRODUCT = "DELETE_CART_PRODUCT";
export const CLEAR_SEARCH_REVIEW = "CLEAR_SEARCH_REVIEW";
export const CLEAR_REVIEWS = "CLEAR_REVIEWS";
export const SEARCH_REVIEW = "SEARCH_REVIEW";
export const CREATE_REVIEWS = "CREATE_REVIEWS";
export const PRODUCT_REVIEWS = "PRODUCT_REVIEWS";
export const REVIEWS_BY_USER = "REVIEWS_BY_USER";
export const DELETE_REVIEW = "DELETE_REVIEW";
export const UPDATE_REVIEW = "UPDATE_REVIEW";
export const GET_ONE_USER = "GET_ONE_USER";
export const GET_INTEL = "GET_INTEL";
export const GET_AMD = "GET_AMD";
export const EDIT_DISCOUNT = "EDIT_DISCOUNT";
export const EDIT_STOCK = "EDIT_STOCK";
export const GET_ALL_FAVORITOS = "GET_ALL_FAVORITOS";
export const DELETE_FAVORITO = "DELETE_FAVORITO";
export const DELETE_ALL_FAVORITO = "DELETE_ALL_FAVORITO";
export const UPDATE_USER = 'UPDATE_USER';
